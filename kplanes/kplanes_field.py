...
def interpolate_ms_features(
    ...
    for grid in grid_encodings:
        grid_features, vol_tv= grid(pts,time_mask,static_mask,dynamic_mask)
        vol_tvs.append(vol_tv)
...
class KPlanesField(Field):
    ...
    def __init__(
        ...
        self.geo_feat_dim = self.grid_select_dim
        ...
        #)
        self.feature_dim = (
            2*grid_select_dim  * len(multiscale_res) if self.concat_across_scales
            else 2*grid_select_dim
        )        
...
class KPlanesDensityField(Field):
    ...
    def get_density(self, ray_samples: RaySamples) -> Tuple[TensorType, None]:
        ...
        positions_flat = positions.view(-1, positions.shape[-1])
        features, vol_tvs = interpolate_ms_features(
            positions_flat, None, None, None, grid_encodings=[self.grids], concat_features=False
        )
        if len(features) < 1:
            features = torch.zeros((0, 1), device=features.device, requires_grad=True)
        density_before_activation = self.sigma_net(features).view(*ray_samples.frustums.shape, -1)
